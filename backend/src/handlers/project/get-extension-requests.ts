...
import { getProjectExtensionRequestsSchema } from "../../schemas/project-schema";
...
export const handler = buildCorsHeaders(async (event: APIGatewayProxyEvent) => {
  ...
  const result = getProjectExtensionRequestsSchema.safeParse(params);
  ...
  const { data, error } = await projectService.getProjectExtensionRequests(result.data);
