import { AboutInfoSchema } from "../../schemas/about-schema";
...
export const handler = buildCorsHeaders(async (event) => {
  try {
    ...
    const parsedData = AboutInfoSchema.safeParse(payload);
    ...
    const { data: updatedSettings, error: upsertError } = await db
      ...
      .upsert(
        { key: "about_info", value: parsedData.data },
        { onConflict: "key" }
      )
      .select()
      .single();
    ...
    if (upsertError) {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ message: "Failed to update about info" }),
      };
    }
    ...
    await logActivity(db, {
      ...
      category: "account",
      target_type: "user",
      details: { message: "Updated about page configuration" },
    });

    return {
      statusCode: 200,
      body: JSON.stringify({
        message: "About page information updated successfully",
