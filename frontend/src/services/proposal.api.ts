...
export const fetchAgencies = async () => {
  ...
  if (error) { console.error("Error loading agencies:", error); return []; }
  return data || [];
};

export const fetchTags = async () => {
  const { data, error } = await supabase.from('tags').select('*').order('name');
  if (error) { console.error("Error loading tags:", error); return []; }
  return data || [];
};

export const fetchStations = async () => {
  const { data, error } = await supabase.from('research_stations').select('*').order('name');
  if (error) { console.error("Error loading stations:", error); return []; }
  return data || [];
};

export const fetchSectors = async () => {
  const { data, error } = await supabase.from('sectors').select('*').order('name');
  if (error) { console.error("Error loading sectors:", error); return []; }
  return data || [];
};

export const fetchDisciplines = async () => {
  const { data, error } = await supabase.from('disciplines').select('*').order('name');
  if (error) { console.error("Error loading disciplines:", error); return []; }
  return data || [];
};
