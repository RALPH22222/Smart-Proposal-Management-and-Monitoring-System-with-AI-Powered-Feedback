...
  CornerDownRight, ChevronUp, ChevronDown // Import Chevron icons
} from 'lucide-react';
...
  // --- NEW PROGRESS INPUT HANDLER ---
  const handleProgressInput = (reportId: number, value: string) => {
    // NOTE: This function is now only called by handleProgressStep for safety
    // It remains here for consistency if manual number input was desired
    const rawValue = parseInt(value, 10);
    const validatedValue = isNaN(rawValue) 
        ? prevReportProgress
        : Math.min(100, Math.max(prevReportProgress, rawValue));

    updateReportField(reportId, 'progressPercentage', validatedValue);
  };
  
  // --- NEW STEPPER HANDLER ---
...
    
    if (direction === 'up') {
      newValue = Math.min(100, currentReport.progressPercentage + 5); // Increment by 5%
    } else {
      newValue = Math.max(prevReportProgress, currentReport.progressPercentage - 5); // Decrement by 5%, constrained by previous progress
    }
