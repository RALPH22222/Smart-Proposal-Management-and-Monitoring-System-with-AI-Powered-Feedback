...
interface QuarterlyReport {
  ...
  quarter: string;
  dueDate: string;
  startDate: string; // New: Report start date
  status: ReportStatus;
  progressPercentage: number;
  fundRequest: { // New: Separate from expense items
    amount: number;
    description: string;
    status: 'pending' | 'approved' | 'rejected';
  } | null;
  expenseItems: ExpenseItem[]; // Actual spending/liquidation
  comments: Comment[];
  proofFiles: string[];
  submittedBy?: string; 
...
interface Project {
  ...
  totalGrant: number; // Renamed from totalBudget
  additionalFunds: number; // New: Funds requested beyond original grant
  startDate: string;
  endDate: string;
...
const MonitoringPage: React.FC = () => {
  ...
  const [projects, setProjects] = useState<Project[]>([
    {
      ...
      reports: [
        { 
          ...
          fundRequest: {
            amount: 250000,
            description: 'Initial setup costs: equipment and personnel',
            status: 'approved'
          },
          expenseItems: [
          ...
          comments: [], 
          proofFiles: ['receipts_q1.pdf', 'site_visit_photos.jpg'],
        ...
        { 
          ...
          dueDate: "Jul 15, 2025", 
          status: 'due', // Changed from submitted - need to submit report
          progressPercentage: 50,
          fundRequest: {
            amount: 300000,
            description: 'Cloud computing and dataset development',
            status: 'approved'
          },
          expenseItems: [
          ...
          comments: [
            { id: 'c1', author: 'R&D Officer', text: "Please submit your Q2 report with progress update and liquidation details.", timestamp: "Jul 16, 2025 09:30 AM" },
          ], 
          proofFiles: [],
        ...
        { 
          ...
          dueDate: "Oct 15, 2025", 
          status: 'fund_request', // Need to request fund first
        ...
        { 
          ...
          dueDate: "Jan 15, 2026", 
          status: 'locked', // Future quarter - locked
          progressPercentage: 0, 
          fundRequest: null,
          expenseItems: [], 
          comments: [], 
    ...
    },
    {
      ...
      reports: [
        { 
          ...
          fundRequest: {
            amount: 50000,
            description: 'Initial survey and assessment phase',
            status: 'approved'
          },
          expenseItems: [{ id: '1', description: 'Survey Forms Printing', amount: 50000 }],
          comments: [], 
          proofFiles: ['survey.pdf'],
        ...
        { 
          id: 2, 
          quarter: "Q2", 
          startDate: "Feb 02, 2025",
          dueDate: "May 01, 2025", 
          status: 'overdue', 
          progressPercentage: 20,
          fundRequest: {
            amount: 100000,
            description: 'Community workshop materials and implementation',
            status: 'approved'
          },
          expenseItems: [],
          comments: [
            ...
            { id: 'c2', author: 'Proponent', text: "We're experiencing delays in community coordination. Will submit by end of week.", timestamp: "May 18, 2025 02:30 PM" }
          ], 
          proofFiles: [] 
        },
        { 
          ...
          dueDate: "Aug 01, 2025", 
          status: 'locked', // Locked because previous quarter is overdue
          progressPercentage: 0,
          fundRequest: null,
          expenseItems: [], 
          comments: [], 
    ...
    },
    {
      ...
      status: 'completed',
      totalGrant: 500000,
      additionalFunds: 100000, // Example of additional funds approved
      ...
      reports: [
        { 
          ...
          fundRequest: {
            amount: 125000,
            description: 'Phase 1: Database design and framework',
            status: 'approved'
          },
          expenseItems: [{id:'1', description:'Server Setup', amount: 125000}], 
          comments: [], 
          proofFiles: ['proof.pdf'],
          submittedBy: "Dr. Albert Einstein", 
        ...
        { 
          ...
          fundRequest: {
            amount: 125000,
            description: 'Phase 2: Data collection tools',
            status: 'approved'
          },
          expenseItems: [{id:'1', description:'Survey Equipment', amount: 125000}], 
          comments: [], 
          proofFiles: ['proof.pdf'],
          submittedBy: "Marine Lab Inc.", 
          dateSubmitted: "Jun 28, 2024"
        },
        { 
          id: 3, 
          quarter: "Q3", 
          ...
          fundRequest: {
            amount: 125000,
            description: 'Phase 3: Database population',
            status: 'approved'
          },
          expenseItems: [{id:'1', description:'Data Processing', amount: 125000}], 
          comments: [], 
          proofFiles: ['proof.pdf'],
          submittedBy: "Dr. Albert Einstein", 
          dateSubmitted: "Sep 29, 2024"
        },
        { 
          id: 4, 
          quarter: "Q4", 
          ...
          fundRequest: {
            amount: 125000,
            description: 'Phase 4: Final deployment and testing',
            status: 'approved'
          },
          expenseItems: [{id:'1', description:'Deployment Costs', amount: 125000}], 
          comments: [], 
          proofFiles: ['proof.pdf'],
          submittedBy: "Dr. Albert Einstein", 
          dateSubmitted: "Dec 30, 2024"
        }
      ]
    }
  ]);
  ...
  useEffect(() => {
    if (activeProject?.id === "2") {
        setCurrentReportIndex(1); // Q2 (due)
    } else if (activeProject?.id === "3") {
        setCurrentReportIndex(1); // Q2 (overdue)
    } else {
  ...
  const totalAvailableFunds = (activeProject?.totalGrant || 0) + (activeProject?.additionalFunds || 0);
  ...
  const submitAdditionalFundRequest = () => {
    if (!additionalFundAmount || !additionalFundJustification.trim()) {
      alert("Please provide amount and justification");
      return;
    }
    ...
    setProjects(prev => prev.map(p => {
      ...
      return {
        ...p,
        additionalFunds: p.additionalFunds + additionalFundAmount
      };
    }));

    setIsAdditionalFundModalOpen(false);
    setAdditionalFundAmount(0);
    setAdditionalFundJustification("");
  ...
  const handleProgressStep = (reportId: number, direction: 'up' | 'down') => {
    ...
    if (direction === 'up') {
      newValue = Math.min(100, currentReport.progressPercentage + 5); 
    } else {
  ...
  const isEditable = currentReport?.status === 'due' || currentReport?.status === 'overdue' || currentReport?.status === 'submitted'; 
  const isOverdue = currentReport?.status === 'overdue';
  const isFundRequestAvailable = activeProject && (activeProject.status === 'active' || activeProject.status === 'delayed');
  ...
  // Function to render chat section
  const renderChatSection = () => {
    if (!currentReport || (currentReport.status === 'locked' && currentReport.comments.length === 0)) return null;
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden shadow-inner mt-6">
            ...
            <div className="p-4 bg-slate-50/50 max-h-64 overflow-y-auto space-y-4">
              {currentReport.comments.length === 0 && (
                <p className="text-center text-xs text-slate-400 italic py-2">No messages yet. Use the box below to communicate with R&D.</p>
              )}
            ...
            {/* Reply Input is only available if report is editable (due, overdue, submitted) */}
            {isEditable && (
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8 flex-1 flex flex-col">
                  ...
                  <div className="mb-8">
                      <div className="flex flex-col sm:flex-row justify-between items-start mb-4 gap-4">
                         ...
                         <div className="flex gap-2 w-full sm:w-auto">
                            ...
                            <button 
                              onClick={() => setIsAdditionalFundModalOpen(true)}
                              className={`flex-1 sm:flex-none justify-center flex items-center gap-2 px-4 py-2 text-white rounded-lg font-semibold shadow-sm text-sm transition-all ${isFundRequestAvailable ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-gray-400 cursor-not-allowed'}`}
                              disabled={!isFundRequestAvailable}
                            >
                              <DollarSign className="w-4 h-4"/> Request Additional Funds
                            </button>
                         </div>
                      </div>
                      ...
                      <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 relative overflow-hidden">
                         <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-2 relative z-10">
                           <div>
                             <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Total Grant</p>
                             <p className="text-xl font-bold text-slate-800">{formatCurrency(activeProject.totalGrant)}</p>
                           </div>
                           <div className="text-center">
                             <p className="text-xs font-bold text-blue-600 uppercase tracking-wide">Additional Funds</p>
                             <p className="text-xl font-bold text-blue-700">{formatCurrency(activeProject.additionalFunds)}</p>
                           </div>
                           <div className="text-right">
                             <p className="text-xs font-bold text-emerald-600 uppercase tracking-wide">Remaining</p>
                             <p className="text-2xl font-black text-emerald-700">{formatCurrency(remainingBudget)}</p>
                           </div>
                         </div>
                         <div className="w-full bg-slate-200 h-3 rounded-full overflow-hidden mb-1">
                           <div className={`h-full transition-all duration-1000 ${budgetProgress > 90 ? 'bg-red-500' : 'bg-emerald-500'}`} style={{ width: `${budgetProgress}%` }}></div>
                         </div>
                         <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase">
                           <span>0%</span>
                           <span>{budgetProgress.toFixed(1)}% Utilized</span>
                         </div>
                ...
                {currentReport && (
                  <div className="flex-1 flex flex-col">
                      ...
                      <div className={`relative rounded-2xl border transition-all animate-in fade-in slide-in-from-right-4 duration-300 ${
                        ...
                        'bg-white border-gray-200 p-4 sm:p-6'
                         ...
                         {isLocked && (
                           <div className="text-center py-8">
                             ...
                             <p className="text-gray-500 mb-4">This quarter hasn't started yet. You can access it starting {currentReport.startDate}.</p>
                             {currentReportIndex > 0 && activeProject.reports[currentReportIndex - 1]?.status === 'overdue' && (
                               <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mt-4">
                                 <p className="text-sm text-amber-700 font-medium">⚠️ Complete the previous quarter first to unlock this quarter.</p>
                               </div>
                             )}
                           </div>
                         )}
...
{isFundRequestNeeded && (
  <div className="border border-blue-200 rounded-xl p-5 bg-blue-50 mt-4">
    ...
    <p className="text-sm text-blue-600 mb-4">
      Before starting this quarter, you need to request funds from the available grant. 
      This will be reviewed by R&D before you can proceed.
    </p>
    ...
    <div className="space-y-3">
      ...
      <div className="border border-gray-200 rounded-xl overflow-hidden">
        <div className="bg-gray-50 px-4 py-3 flex justify-between items-center border-b border-gray-200">
          ...
          <button 
            onClick={() => setBreakdownItems([...breakdownItems, { id: Date.now().toString(), description: '', amount: 0 }])}
        ...
        <div className="p-4 space-y-3 bg-white">
          {breakdownItems.length === 0 && (
            <p className="text-sm text-gray-400 text-center py-4 italic">No breakdown items added yet. Please add planned expenditures.</p>
          )}
          {breakdownItems.map((item) => (
            <div key={item.id} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
              <input
                ...
                placeholder="Description (e.g., Equipment, Travel, Supplies)"
              />
              <div className="flex w-full sm:w-auto gap-2">
                <input
                  type="number"
                  value={item.amount || ''}
                  onChange={(e) => updateBreakdownItem(item.id, 'amount', parseFloat(e.target.value))}
                  className="flex-1 sm:w-24 p-2 border border-gray-300 rounded text-sm focus:ring-1 focus:ring-blue-500 outline-none text-right"
                  placeholder="Amount"
                />
                <button
                  onClick={() => removeBreakdownItem(item.id)}
                  className="text-gray-400 hover:text-red-500 p-2 sm:p-0"
                >
      ...
      {breakdownItems.length > 0 && (
        <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
          <div className="flex justify-between items-center">
            <span className="text-sm font-bold text-blue-700">Total Amount Requested:</span>
      ...
      <button
        onClick={() => submitFundRequest(currentReport.id, breakdownItems)}
        disabled={breakdownItems.length === 0 || breakdownItems.some(item => !item.description || !item.amount)}
        className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-md transition-all disabled:opacity-50 disabled:cursor-not-allowed"
      >
        Submit Fund Request for R&D Review
      </button>
    </div>
  </div>
)}
                         {/* EDITABLE CONTENT (Appears when status is 'due', 'overdue', or 'submitted') */}
                         {isEditable && (
                           <div className="space-y-6">
                             {/* Approved Fund Request Display */}
                             {currentReport.fundRequest && currentReport.fundRequest.status === 'approved' && (
                               <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                                 <div className="flex justify-between items-center mb-2">
                                   ...
                                   <span className="text-lg font-bold text-emerald-700">{formatCurrency(currentReport.fundRequest.amount)}</span>
                                 </div>
                                 <p className="text-sm text-emerald-700">{currentReport.fundRequest.description}</p>
                               </div>
                             )}

                             {/* Progress Input/Stepper */}
                             <div className="bg-white p-4 rounded-xl border border-gray-200">
                               <label className="text-xs font-bold text-gray-600 uppercase mb-3 flex justify-between items-center">
                                 ...
                                 <span className="text-sm font-bold text-blue-600">{currentReport.progressPercentage}%</span>
                               </label>
                               
                               {/* PROGRESS STEPPER */}
                               <div className="flex items-center gap-3">
                                   <div 
                                     className="flex-1 w-full p-3 border border-gray-300 rounded-lg text-sm bg-gray-50 text-gray-800 font-semibold text-center select-none"
                                   >
                                     {currentReport.progressPercentage}%
                                   </div>
                                   <div className="flex flex-col gap-1">
                                     <button 
                                       onClick={() => handleProgressStep(currentReport.id, 'up')}
                                       disabled={currentReport.progressPercentage >= 100}
                                       className="p-1 bg-gray-200 rounded-lg hover:bg-blue-100 text-gray-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                                     >
                                       <ChevronUp className="w-4 h-4"/>
                                     </button>
                                     <button 
                                       onClick={() => handleProgressStep(currentReport.id, 'down')}
                                       disabled={currentReport.progressPercentage <= prevReportProgress}
                                       className="p-1 bg-gray-200 rounded-lg hover:bg-red-100 text-gray-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                                     >
                               ...
                               <div className="mt-4">
                                 <div className="w-full bg-gray-200 rounded-full h-2">
                                   <div className="bg-blue-600 h-2 rounded-full transition-all duration-300" style={{ width: `${currentReport.progressPercentage}%` }}></div>
                                 </div>
                               </div>
                             </div>

                             {/* Expense Breakdown (Liquidation Log) */}
{/* Expense Breakdown (Liquidation Proof Upload) */}
<div className="border border-gray-200 rounded-xl overflow-hidden">
  <div className="bg-gray-50 px-4 py-3 flex justify-between items-center border-b border-gray-200">
    <span className="text-xs font-bold text-gray-600 uppercase">Expense Liquidation (Proof of Spending)</span>
  </div>
  <div className="p-4 space-y-3 bg-white">
    {/* Show approved fund request breakdown */}
    {currentReport.fundRequest && currentReport.fundRequest.status === 'approved' && (
      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 mb-4">
        <p className="text-xs font-bold text-emerald-700 uppercase mb-2">Approved Fund Request:</p>
        <p className="text-sm text-emerald-800 whitespace-pre-line">{currentReport.fundRequest.description}</p>
        <p className="text-sm font-bold text-emerald-700 mt-2 text-right">
          Total: {formatCurrency(currentReport.fundRequest.amount)}
        </p>
      </div>
    )}
    
    <p className="text-xs text-gray-500 italic mb-2">
      Upload proof of spending (receipts, invoices, photos) corresponding to the approved fund request above.
      The amounts should match the approved breakdown.
    </p>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-center w-full">
        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors">
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            <p className="text-xs text-gray-400">PNG, JPG, PDF (max. 10MB each)</p>
          </div>
          <input 
            type="file" 
            multiple 
            ...
            onChange={(e) => { 
              if(e.target.files) {
                ...
                updateReportField(currentReport.id, 'proofFiles', [...currentReport.proofFiles, ...newFiles]); 
              }
            }}
          />
        </label>
      ...
      {currentReport.proofFiles.length > 0 ? (
        <div className="space-y-2">
          <p className="text-xs font-bold text-gray-600">Uploaded Files:</p>
          <div className="space-y-2">
            {currentReport.proofFiles.map((file, i) => (
              <div key={i} className="flex items-center justify-between bg-gray-50 border border-gray-200 px-3 py-2 rounded-lg">
                <div className="flex items-center gap-2">
                  <FileText className="w-4 h-4 text-blue-500" />
                  <span className="text-xs text-gray-700 truncate max-w-[200px]">{file}</span>
                </div>
                <button 
                  type="button"
                  onClick={() => {
                    const updatedFiles = [...currentReport.proofFiles];
                    updatedFiles.splice(i, 1);
                    updateReportField(currentReport.id, 'proofFiles', updatedFiles);
                  }}
        ...
        <p className="text-xs text-gray-400 italic text-center">No files uploaded yet.</p>
      )}
    </div>
  </div>
</div>
 ...
 disabled={currentReport.proofFiles.length === 0}                             >
                         ...
                         {currentReport.status === 'approved' && (
                           <div className="border-t border-gray-100 pt-4">
                             ...
                             {currentReport.expenseItems.length > 0 ? (
                               <div className="mb-4">
                                  <p className="text-xs font-bold text-gray-500 uppercase mb-2">Verified Expense Liquidation</p>
                                  <div className="bg-gray-50 rounded-xl p-3 space-y-1">
                                     ...
                                     <div className="flex justify-between text-xs font-bold text-gray-700 pt-2 mt-2 border-t border-gray-300">
                                       <span>Total</span>
                                       <span className="font-mono">{formatCurrency(reportTotal)}</span>
                                     </div>
                                  </div>
                               </div>
                             ) : <p className="text-xs text-gray-400 italic mb-4">No expenses recorded for this quarter.</p>}
                             
                             {/* Proof of Accomplishment (Read-only) */}
                             <p className="text-xs font-bold text-gray-500 uppercase mb-2">Proof of Accomplishment & Receipts</p>
                             ...
                             {(currentReport.submittedBy && currentReport.dateSubmitted) && (
      ...
      {/* --- MODALS (Additional Funds & Extension) --- */}
      {isAdditionalFundModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-in fade-in">
           <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 relative animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
              <button onClick={() => setIsAdditionalFundModalOpen(false)} className="absolute right-4 top-4 text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
              <div className="text-center mb-6">
                 <div className="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-3"><DollarSign className="w-6 h-6" /></div>
                 <h3 className="text-xl font-bold text-gray-800">Request Additional Funds</h3>
                 <p className="text-sm text-gray-500">Request funds beyond the original grant amount with justification.</p>
              </div>
              <div className="space-y-4">
                 <div>
                   <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Amount Requested (PHP)</label>
                   <input 
                     type="number" 
                     value={additionalFundAmount || ""} 
                     onChange={(e) => setAdditionalFundAmount(parseFloat(e.target.value))}
                     className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                     placeholder="Enter amount"
                   />
                 </div>
                 <div>
                   <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Justification</label>
                   <textarea 
                     value={additionalFundJustification}
                     onChange={(e) => setAdditionalFundJustification(e.target.value)}
                     className="w-full p-3 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none h-32 resize-none"
                     placeholder="Explain why additional funds are needed..."
                   />
                 </div>
                 <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                   <p className="text-center text-sm font-bold text-emerald-800 uppercase">Total Project Funds After Approval</p>
                   <p className="text-center text-3xl font-black text-emerald-600">
                     {formatCurrency((activeProject?.totalGrant || 0) + (activeProject?.additionalFunds || 0) + additionalFundAmount)}
                   </p>
                 </div>
              </div>
              <div className="mt-6">
                <button 
                  onClick={submitAdditionalFundRequest} 
                  disabled={!additionalFundAmount || additionalFundAmount <= 0 || !additionalFundJustification.trim()}
                  className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-lg transition-all disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
                  Submit Additional Fund Request
                </button>
              </div>
           </div>
        </div>
