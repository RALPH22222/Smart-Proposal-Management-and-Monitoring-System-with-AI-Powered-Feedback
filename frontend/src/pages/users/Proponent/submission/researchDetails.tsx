...
// --- IMPORT API SERVICES (Assuming these exist and return { id: number, name: string }[]) ---
import { 
  fetchStations, 
  fetchSectors, 
  fetchDisciplines, 
...
  // Search term state controls filtering, but input value is still read from formData.
...
        // Assuming services return array of { id, name } objects
...
    setDisciplineSearch(formData.discipline || '');

  }, [formData.priorityAreas, formData.researchType, formData.developmentType, 
    formData.researchStation, formData.sectorCommodity, formData.discipline]);
...
  // --- 4. IMPLEMENTATION SITES LOGIC (UNCHANGED) ---
...
  // --- 5. GENERAL HANDLERS (ENHANCED FOR TYPING) ---

  // Handles selecting a value from the dropdown list
...
  // Handles manual typing in the input field
  const handleTextChange = (name: keyof FormData, value: string, setSearch: (s: string) => void) => {
    setSearch(value);
    // Update form data immediately to reflect the typing
    onUpdate(name, value);
  };

  // Handles blur event to ensure even custom typed text is explicitly saved
  const handleTextBlur = (name: keyof FormData, value: string, setOpen: (b: boolean) => void) => {
    onUpdate(name, value.trim());
    setOpen(false);
  };
  
  // --- Research & Development Type Handlers (UNCHANGED) ---
...
  // Click Outside Listener (UNCHANGED)
  useEffect(() => {
...
            value={researchStationSearch} // Use local search state
...
              value={sectorSearch} // Use local search state
...
              type="text"
              name="discipline"
              value={disciplineSearch} // Use local search state
...
              onBlur={() => setIsPriorityOpen(false)} // Blur closes dropdown
