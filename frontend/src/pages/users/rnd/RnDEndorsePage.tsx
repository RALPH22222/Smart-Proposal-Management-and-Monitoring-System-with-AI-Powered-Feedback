...
import EndorsementDecisionModal from "../../../components/rnd-component/EndorsementDecisionModal";
...
const EndorsePage: React.FC = () => {
  ...
  const [selectedProposalForView, setSelectedProposalForView] = useState<{
    title: string;
    id: string;
  } | null>(null);
  const [isViewModalOpen, setIsViewModalOpen] = useState(false);

  const [selectedProposalForAction, setSelectedProposalForAction] = useState<{
    title: string;
    id: string;
  } | null>(null);
  ...
  // Handler for viewing individual evaluator decisions
  const handleOpenViewModal = (
    ...
    setSelectedProposalForView({ title: proposalTitle, id: proposalId });
  ...
  // Handler for opening the Action modal (EndorsementDecisionModal)
  const handleOpenActionModal = (proposalTitle: string, proposalId: string) => {
    setSelectedProposalForAction({ title: proposalTitle, id: proposalId });
  ...
  // Handler for submitting the endorsement decision from the modal
  const handleSubmitAction = async (
    status: "endorsed" | "returned" | "rejected",
    remarks: string
  ) => {
    if (!selectedProposalForAction) return;

    const proposalId = selectedProposalForAction.id;

    try {
      // Simulate API call
      console.log(`Processing decision for ${proposalId}: ${status}`);
      ...
      console.log(`Proposal ${proposalId} processed successfully as ${status}`);
  ...
  const getOverallRecommendationBadge = (recommendation: string) => {
    const baseClasses =
      "inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border border-current border-opacity-20";
  ...
  const paginatedProposals = endorsementProposals.slice(
    startIndex,
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen lg:h-screen flex flex-col lg:flex-row">
      {/* Modal for viewing individual evaluator decisions */}
      <EvaluatorDecisionModal
        isOpen={isViewModalOpen}
        onClose={handleCloseViewModal}
        decision={selectedDecision!}
        proposalTitle={selectedProposalForView?.title || ""}
        proposalId={selectedProposalForView?.id || ""}
      />

      {/* Modal for taking action (Endorse/Return/Reject) */}
      {isActionModalOpen && selectedProposalForAction && (
        <EndorsementDecisionModal
          isOpen={isActionModalOpen}
          onClose={handleCloseActionModal}
          proposalTitle={selectedProposalForAction.title}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-slate-600 mt-2 text-sm leading-relaxed">
                Review evaluator decisions and endorse proposals for final
                approval
              </p>
            </div>
            ...
            {endorsementProposals.length === 0 ? (
              <div className="text-center py-12 px-4">
                ...
                <p className="text-slate-500 max-w-sm mx-auto">
                  Proposals will appear here once evaluators complete their
                  reviews.
                </p>
              </div>
            ) : (
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start gap-3 mb-3">
                            ...
                            <span
                              className={getOverallRecommendationBadge(
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                              {proposal.evaluatorDecisions.map((decision) => (
                                <div
                                  ...
                                  className={`border rounded-lg p-3 ${getDecisionColor(
                                    decision.decision
                              ...
                              {proposal.evaluatorDecisions.length < 2 && (
                                <div className="border-2 border-dashed border-slate-300 rounded-lg p-4 flex items-center justify-center">
                                  <div className="text-center text-slate-500">
                                    ...
                                    <p className="text-xs">
                                      Cannot endorse until all evaluators
                                      complete review
                                    </p>
                                  </div>
                                </div>
                        ...
                        {proposal.readyForEndorsement && (
                          <div className="flex flex-col sm:flex-row gap-2 flex-shrink-0">
                            {/* Replaced the 3 separate buttons with 1 Action button */}
                            <button
                              onClick={() =>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex-shrink-0">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-xs text-slate-600">
                <span>
                  ...
                  of {endorsementProposals.length} proposals
                </span>
