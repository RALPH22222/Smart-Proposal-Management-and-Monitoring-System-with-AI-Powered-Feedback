...
const AdminFundingPage: React.FC = () => {
  ...
  const loadFundingProposals = async () => {
    try {
      ...
      // Fetch raw data from the endorsement archive (which includes endorsed and funded statuses)
      const rawData = await (await import('../../../services/proposal.api')).getProposalsForEndorsement('archive');

      // Fetch mapped proposals using Admin API
      const allMapped = await adminProposalApi.fetchProposals();
