...
const PriorityBadge: React.FC<{ priority: "low" | "medium" | "high" | "urgent" }> = ({ priority }) => {
  ...
  return (
    <span className={`px-2 py-1 text-xs font-medium rounded-md border ${styles[priority]} capitalize`}>{priority}</span>
  )
}
...
function Reviews() {
  ...
  type SortKey = keyof Pick<
    CommitteeReview,
    "id" | "itemTitle" | "status" | "priority" | "submittedAt" | "reviewDeadline"
  >
  ...
  const filtered = useMemo(() => {
    return mockCommitteeReviews.filter((review) => {
      ...
      const matchPriority = priority === "all" ? true : review.priority === priority
      const matchItemType = itemType === "all" ? true : review.itemType === itemType
  ...
  const sorted = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      const av: any = a[sortKey as keyof CommitteeReview]
      const bv: any = b[sortKey as keyof CommitteeReview]
  ...
  const stats = useMemo(() => {
    const pending = mockCommitteeReviews.filter((r) => r.status === "pending").length
    const underReview = mockCommitteeReviews.filter((r) => r.status === "under_review").length
    const approvedThisWeek = mockCommitteeReviews.filter(
      (r) => r.status === "approved" && new Date(r.decidedAt || "").getTime() > Date.now() - 7 * 24 * 60 * 60 * 1000,
    ).length
    const overdue = mockCommitteeReviews.filter(
      (r) => r.status === "pending" && new Date(r.reviewDeadline).getTime() < Date.now(),
    ).length
      ...
      <main className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          <header className="mb-6 flex items-start md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Committee Reviews</h1>
              <p className="text-gray-600 mt-1">Track committee feedback and decision logs for institutional items.</p>
            </div>
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-white rounded-lg shadow p-4">
              ...
              <div className="mt-2 text-xs text-gray-400">Awaiting committee action</div>
            </div>
            <div className="bg-white rounded-lg shadow p-4">
              <div className="text-xs text-gray-500">Under Review</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">{stats.underReview}</div>
              <div className="mt-2 text-xs text-gray-400">Active committee review</div>
            </div>
            <div className="bg-white rounded-lg shadow p-4">
              <div className="text-xs text-gray-500">Approved (7d)</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">{stats.approvedThisWeek}</div>
              <div className="mt-2 text-xs text-gray-400">Last 7 days</div>
            </div>
            <div className="bg-white rounded-lg shadow p-4">
              <div className="text-xs text-gray-500">Overdue Items</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">{stats.overdue}</div>
                ...
                <thead>
                  <tr className="text-left text-xs text-gray-500 border-b">
                    ...
                    <th className="px-4 py-3">
                      ...
                      >
                        Item{sortKey === "itemTitle" ? (sortDir === "asc" ? " ▲" : " ▼") : ""}
                      </button>
                    </th>
                    ...
                    <th className="px-4 py-3">
                      ...
                      >
                        Deadline{sortKey === "reviewDeadline" ? (sortDir === "asc" ? " ▲" : " ▼") : ""}
                      </button>
                    </th>
                    ...
                    <tr key={review.id} className="border-b last:border-b-0">
                      ...
                      <td className="px-4 py-3">
                        <div className="max-w-xs">
                          <div className="text-sm font-medium text-gray-800 truncate">{review.itemTitle}</div>
                          <div className="text-xs text-gray-500 truncate capitalize">
                            {review.itemType} • {review.submittedBy}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-700">{review.assignedCommittee}</td>
                      <td className="px-4 py-3">
              ...
              {paginated.map((review) => (
                <div key={review.id} className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="text-sm font-semibold text-gray-900">{review.itemTitle}</div>
                  ...
                  <div className="mt-2 text-xs text-gray-600">
                    Submitted by {review.submittedBy} • Deadline: {new Date(review.reviewDeadline).toLocaleDateString()}
                  </div>
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <div className="text-xs text-gray-500">
                      Last activity: {new Date(review.lastActivity).toLocaleDateString()}
                    </div>
                  </div>
