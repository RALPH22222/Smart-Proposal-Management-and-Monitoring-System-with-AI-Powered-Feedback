            ...
            <div className="p-4 space-y-5">
              ...
              <div>
                <h4 className="font-bold text-sm text-slate-700 mb-3 flex items-center gap-2">
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-red-100 text-red-600 text-[10px] font-extrabold">S</span>
                  Scoring Criteria (1 – 5)
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-5 gap-2">
                  {rubric.criteria.slice().sort((a, b) => b.score - a.score).map((crit, critIdx) => (
                    <div key={critIdx} className="flex flex-col gap-1.5 bg-slate-50 border border-slate-200 rounded-lg p-2">
                      <div className="flex items-center gap-1.5">
                        <div className="w-6 h-6 flex-shrink-0 rounded-md bg-red-600 text-white font-extrabold flex items-center justify-center text-xs shadow-sm">
                          {crit.score}
                        </div>
                        <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                          {crit.score === 5 ? 'Excellent' : crit.score === 4 ? 'Very Good' : crit.score === 3 ? 'Good' : crit.score === 2 ? 'Fair' : 'Poor'}
                        </span>
                      </div>
                      <textarea
                        ...
                        rows={3}
                        placeholder="Describe this score level..."
                        className="w-full bg-white border border-slate-200 rounded px-2 py-1.5 text-[11px] leading-relaxed focus:outline-none focus:border-red-400 resize-none text-slate-700"
                      />
                    </div>
                  ))}
                </div>
              ...
              {/* Evaluator Guide Questions */}
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-bold text-sm text-slate-700 flex items-center gap-2">
                    <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-blue-100 text-blue-600 text-[10px] font-extrabold">Q</span>
                    Evaluator Guide Questions
                    <span className="ml-1 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-500 border border-slate-200">
                      {rubric.evaluatorGuide.length}
                    </span>
                  </h4>
                  <button
                    onClick={() => addGuide(catIdx)}
                    className="text-xs font-bold text-red-600 hover:text-red-800 flex items-center gap-1 px-2 py-1 rounded-md hover:bg-red-50 transition-colors border border-red-200"
                  >
                ...
                {rubric.evaluatorGuide.length === 0 ? (
                  <p className="text-xs text-slate-400 italic py-3 text-center border border-dashed border-slate-200 rounded-lg bg-slate-50">
                    No guide questions defined. Click "+ Add Question" to add one.
                  </p>
                ) : (
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-1 custom-scrollbar">
                    {rubric.evaluatorGuide.map((guide, guideIdx) => (
                      <div key={guideIdx} className="flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
                        <span className="text-blue-400 font-bold text-sm flex-shrink-0">{guideIdx + 1}.</span>
                        <input
                          type="text"
                          value={guide}
                          onChange={(e) => updateGuide(catIdx, guideIdx, e.target.value)}
                          placeholder="Enter guide question..."
                          className="flex-1 bg-transparent border-none outline-none text-xs text-slate-700 placeholder-slate-400 font-medium"
                        />
                        <button
                          onClick={() => removeGuide(catIdx, guideIdx)}
                          className="text-red-300 hover:text-red-600 flex-shrink-0 p-1 rounded hover:bg-red-50 transition-colors"
