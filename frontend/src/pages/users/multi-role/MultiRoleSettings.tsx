...
const Settings: React.FC = () => {
  ...
  const displayName = profile?.name || `${formData.firstName} ${formData.lastName}`.trim() || 'User Profile';
  ...
  const displayUserRoles = orderedRoles.map((r: string) => roleLabels[r.toLowerCase()] || (r.charAt(0).toUpperCase() + r.slice(1))).join(', ');
        ...
        <div className="bg-gradient-to-br from-[#C8102E] to-[#8B0C20] rounded-2xl p-6 mb-6 text-white shadow-lg">
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-5">
            ...
            <div className="text-center sm:text-left flex-1">
              ...
              <p className="text-red-200 text-sm mt-0.5">{profile?.email || 'â€”'}</p>
            ...
            <div className="flex sm:flex-col items-center gap-4 sm:gap-2 bg-white/10 rounded-xl px-4 py-3 text-center">
              <div>
                <div className="text-xl font-bold">{displayUserRoles}</div>
                <div className="text-red-200 text-xs">Account Roles</div>
              </div>
            </div>
          </div>
        </div>
