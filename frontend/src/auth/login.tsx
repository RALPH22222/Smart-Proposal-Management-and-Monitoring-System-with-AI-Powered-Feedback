...
export default function Login() {
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!email || !password) return Swal.fire({ icon: "warning", title: "Missing fields", text: "Please provide email and password." });
    ...
    try {
      ...
      Swal.fire({ icon: "success", title: "Logged in", text: data.message || "Successfully signed in." });
      setEmail(""); setPassword("");
    } catch (err: any) {
      Swal.fire({ icon: "error", title: "Error", text: err.message || "Login failed" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    if (!token) return Swal.fire({ icon: "error", title: "Google login failed", text: "No credential returned." });
