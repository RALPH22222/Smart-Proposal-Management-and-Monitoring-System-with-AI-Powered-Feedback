    ...
    <div className="fixed bottom-6 right-6 z-50" ref={notifRef}>
      <button
        ...
        className={`relative p-4 rounded-full shadow-2xl transition-all duration-300 transform hover:scale-110 active:scale-95 flex items-center justify-center group ${
          notificationsOpen ? "bg-[#C8102E] text-white rotate-12" : "bg-white text-slate-600 hover:text-[#C8102E]"
        }`}
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 flex h-6 w-6">
            ...
            <span className="relative inline-flex rounded-full h-6 w-6 text-white text-[10px] font-bold items-center justify-center border-2 border-white bg-[#C8102E]">
        ...
        {!notificationsOpen && (
          <div className="absolute right-full mr-4 px-3 py-2 bg-slate-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap shadow-xl">
            Notifications
            <div className="absolute top-1/2 -right-1 -translate-y-1/2 border-y-4 border-y-transparent border-l-4 border-l-slate-800"></div>
          </div>
        )}
      </button>
      ...
      <div className="absolute right-0 bottom-full mb-4">
        <NotificationsDropdown
          ...
          onNavigate={(link) => {
            // Notifications often route by simple string links. This sends it back to the MultiRole router logic to interpret.
            onPageChange(link);
            setNotificationsOpen(false);
          }}
          position="bottom"
...
export default MultiRoleFloatingNotification;
