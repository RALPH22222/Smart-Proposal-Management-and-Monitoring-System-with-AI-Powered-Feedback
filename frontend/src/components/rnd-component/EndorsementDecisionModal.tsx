...
export default function DecisionModal({
  ...
  const [decision, setDecision] = useState<"endorsed" | "returned" | "rejected">("endorsed");
  
  // State for simple remarks (Endorse/Reject)
  const [remarks, setRemarks] = useState("");
  
  // State for structured remarks (Return)
  ...
  const handleSubmit = () => {
    ...
    onSubmit(decision, finalRemarks);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 flex-shrink-0 flex items-center justify-end gap-3">
          ...
          >
            {decision === "endorsed" ? "Confirm Endorsement" : decision === "returned" ? "Send for Revision" : "Confirm Rejection"}
          </button>
        </div>
      </div>
    </div>
